...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() != 2 {
        eprintln!("Usage: test_threshold_logic.rs <file.fastq.gz>");
        std::process::exit(1);
    }

    let file_path = &args[1];
    
    // Test the regular library interface (should use threshold logic)
    ...
    let stream = FastqStream::from_path(file_path)?;
